...
describe('parse', function() {
  ...
  it('uses locals instead of scope when there is a mathing key', function() {
    var fn = parse('aKey');
    expect(fn({aKey: 42}, {aKey: 43})).toBe(43);
  });

  it('does not use locals instead of scope when no mathing key', function() {
    var fn = parse('aKey');
  ...
  it('uses locals when there is a matching locals 4-part key', function() {
  ...
  it('parses a function call with a single indentifier argument', function() {
  ...
  it('calls function accessed as properties with the correct this', function() {
    var scope = {
      anObject: {
        aMember: 42,
        aFunction: function() {
  ...
  it('parses relation operators', function() {
  ...
  it('parses quality operators', function() {
  ...
  it('parses the ternary expression', function() {
    ...
    expect(parse('a === 42 ? true : false')({a: 32})).toBe(false);
  });
  ...
  it('makes ternaries constant if their operands are', function() {
    expect(parse('true ? 42 : 43').constant).toBeTruthy();
    expect(parse('true ? 42 : a').constant).toBeFalsy();
  });

  it('parses parenthese alrering precedence order', function() {
